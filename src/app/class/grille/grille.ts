export class Grille {
  ...
  private calculate(symbole: 1 | 5) {
    let somme = symbole * 3;
    if (
      this.calculRow(0) == somme ||
      this.calculRow(1) == somme ||
      this.calculRow(2) == somme ||
      this.calculCol(0) == somme ||
      this.calculCol(1) == somme ||
      this.calculCol(2) == somme ||
      this.calculDiag(1) == somme ||
      this.calculDiag(2) == somme
    ) {
      this.win = symbole;
      this.endManche = true;
  ...
  private calculDiag(range: 1 | 2) {
    return range == 1
      ? this.case[0][0] + this.case[1][1] + this.case[2][2]
      : this.case[0][2] + this.case[1][1] + this.case[2][0];
  }
  ...
  grilleToPlateau(_case: (0 | 1 | 5)[][]): number[] {
    const plateau: number[] = [];
    _case.forEach((g) => {
      plateau.push(...g);
    });
    return plateau.map((n) => (n === 1 ? -1 : n === 0 ? 0 : 1));
  }

  indexToPosition(_index: number): [number, number] {
    return [Math.floor(_index / 3), _index % 3];
  }
}
